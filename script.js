...
///////////////////////////////////////////////////////////////////////////////////////////////////
//graph problem - consists of nodes and edges
//directed graphs - one way relation between nodes
//undirected graphs - 
//weighted graphs - nodes have a different type of weight (has a cycle, return to where it started)
//unweighted graphs - (has a cycle)
...
//2. find the transpose of the graph (unweighted)
const transpose = {};
for(const outerBag of Object.keys(bagGraph)) {
  ...
  for(const innerBag in innerBags) {
    if(!(innerBag in transpose)) {
      transpose[innerBag] = new Set();
    }
    transpose[innerBag].add(outerBag); 
  }
}
// console.log(transpose);

//3.  write an algorithm to find all ancestors
const findAllParents = (bagColorString, visited = new Set()) => {
  visited.add(bagColorString);
  const parentSet = transpose[bagColorString] || new Set();
  for(const parent of parentSet) {
    if(!visited.has(parent)) {
      findAllParents(parent, visited)
    }
  }
